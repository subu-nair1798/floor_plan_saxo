...
export function save_data () {
  ...
  for(let i = 0; i < 2; i++) {
    let bay_ar = []; 
    for(let j = 0; j < 3; j++) {
      bay_ar[j] = "F"+parseInt(i+1)+"-B"+parseInt(j+1);
    }
    floor_ar.push({
        floor_id: "F"+parseInt(i+1),
        bay_ar: bay_ar
    })
  }
  ...
  for(let i = 0; i < 2; i++) {
    for(let j = 0; j < 3; j++) {
      let seat_ar = [];
      for(let k = 0; k < 5; k++) {
        seat_ar[k] = floor_ar[i].bay_ar[j]+"-S"+parseInt(k+1)
      }
      bay_ar.push({
  ...
  for(let j = 0; j < 6; j++) {
    for(let k = 0; k < 5; k++) {
      seat_ar.push({
        ...
        emp_id: emp_ar[emp_count].emp_id,
        assign_date: "null"
      })
      emp_count++;
    }
  ...
  // console.log(emp_plan)
  // console.log(floor_plan);
  // console.log(bay_plan);
